/* ================================================================
   ...
   5. Collapsible filter panel — toggle open/close
   6. Price Range slider — live output display
   7. Filter — search & reset handlers
   8. Wishlist — save/unsave toggle on cards
   ================================================================ */
...
(function initFilterPanel() {
  ...
  // Close when clicking outside the filter section
  document.addEventListener('click', (e) => {
    const section = document.getElementById('search');
    if (section && !section.contains(e.target)) closePanel();
  });
})();
...
(function initPriceSlider() {
  ...
  output.textContent = format(slider.value); // init
})();


/* ----------------------------------------------------------------
   7. FILTER — search & reset handlers
   Add or rename filter IDs here to match your HTML.
---------------------------------------------------------------- */
(function initFilterActions() {
  ...
  if (searchBtn) {
    searchBtn.addEventListener('click', () => {
      const one   = document.getElementById('filterOne')?.value   || '';
      const two   = document.getElementById('filterTwo')?.value   || '';
      const three = document.getElementById('filterThree')?.value || '';
      const price = document.getElementById('filterPrice')?.value || '';

      const filters = { one, two, three, maxPrice: price };
      console.log('Filter search triggered:', filters);

      // Scroll to content after search
      const featured = document.getElementById('featured');
      if (featured) featured.scrollIntoView({ behavior: 'smooth' });

      // TODO: fetch('/api/items?' + new URLSearchParams(filters))
  ...
  if (resetBtn) {
    resetBtn.addEventListener('click', () => {
      ['filterOne', 'filterTwo', 'filterThree'].forEach(id => {
      ...
      if (output) { output.textContent = 'Any Price'; }
    });
  }
})();
...
/* ----------------------------------------------------------------
   7. WISHLIST TOGGLE — heart icon on content cards
